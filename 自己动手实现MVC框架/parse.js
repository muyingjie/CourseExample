...
function getTokens (text){
    ...
    while (index < text.length){
        ...
        if (isNumber(curCharacter) || (curCharacter == '.' && isNumber(getNextCharacter()))) {
            ...
            // 处理操作符
            var ch = curCharacter;
            var ch2 = curCharacter + getNextCharacter(1);
            var ch3 = curCharacter + getNextCharacter(1) + getNextCharacter(2);
            var op = OPERATORS[ch];
            var op2 = OPERATORS[ch2];
            var op3 = OPERATORS[ch3];
            if (op || op2 || op3) {
                // 这样写的可读性比较差
                // var token = op ? ch : op2 ? ch2 : op3 ? ch3 : "";
    ...
    function getNextCharacter(n){
        // return index < text.length - 1 ? text.charAt(index + 1) : false;
        n = n || 1;
        return index + n < text.length ? text.charAt(index + n) : false;
    }
...
function getAST (tokens){
    ...
    function getAssignmentNode (){
        var left = getTernaryNode();
        if (_expectThenShift("=")) {
            return {
                type: astType.AssignmentExpression,
                left: left,
                right: getTernaryNode()
            };
        }
        return left;
    }

    function getTernaryNode (){
        var test = getLogicalOrNode();
        if (_expectThenShift("?")) {
            var consequent = getAssignmentNode();
            if (_expectThenShift(":")) {
                var alternate = getAssignmentNode();
                return {
    ...
    function getLogicalOrNode (){
        var left = getLogicalAndNode();
        var token;
        while(token = (_expectThenShift('||'))) {
            left = {
                type: astType.BinaryExpression,
                left: left,
                operator: token.text,
                right: getLogicalAndNode()
            };
        }
        return left;
    }

    function getLogicalAndNode (){
        var left = getEqualityNode();
        var token;
        while(token = (_expectThenShift('&&'))) {
            left = {
                type: astType.BinaryExpression,
                left: left,
                operator: token.text,
                right: getEqualityNode()
            };
        }
        return left;
    }

    function getEqualityNode (){
        var left = getRelationalNode();
        var token;
        while(token = (_expectThenShift('==')) || (_expectThenShift('!=')) || (_expectThenShift('===')) || (_expectThenShift('!=='))) {
            left = {
                type: astType.BinaryExpression,
                left: left,
                operator: token.text,
                right: getRelationalNode()
            };
        }
        return left;
    }

    function getRelationalNode (){
        var left = getAdditiveNode();
        var token;
        while(token = (_expectThenShift('>=')) || (_expectThenShift('<=')) || (_expectThenShift('<')) || (_expectThenShift('>'))) {
            left = {
                type: astType.BinaryExpression,
                left: left,
                operator: token.text,
                right: getAdditiveNode()
            };
        }
        return left;
    }

    function getAdditiveNode (){
        var left = getMultiplicativeNode();
        var token;
        while(token = (_expectThenShift('+')) || (_expectThenShift('-'))) {
            left = {
                type: astType.BinaryExpression,
                left: left,
                operator: token.text,
    ...
    function getPrimaryNode (){
        // 基础类型节点
        // 需要最先解析改变优先级的括号，这样primary最开始被赋值的这部分才会在叶子节点中
        if (_expectThenShift('(')) {
            primaryNode = getAssignmentNode();
            _expectThenThrow(')');
    ...
    function _recurseParseAST (ast, oFormalParentObjInfo, isCreate){
        ...
        switch (ast.type){
            ...
            case astType.LogicalExpression:
                return "(" + _recurseParseAST(ast.left) + ")" +
                    ast.operator +
                    "(" + _recurseParseAST(ast.right) + ")";
            case astType.ConditionalExpression:
                intoId = _addFragmentOfGetNextVariableId();
                // testId变量用于存储条件值
                var testId = _addFragmentOfGetNextVariableId();
                aWatchFnFragment.push(testId + "=" + _recurseParseAST(ast.test) + ";");
                _addFragmentOfCreateIf(testId, intoId + "=" + _recurseParseAST(ast.consequent) + ";");
                _addFragmentOfCreateIf("!" + testId, intoId + "=" + _recurseParseAST(ast.alternate) + ";");
                return intoId;
        }
    }
    // 常量转义
